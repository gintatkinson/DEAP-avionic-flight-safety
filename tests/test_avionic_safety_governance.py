...
def test_repository_structure_exists():
    ...
    assert (REPO_ROOT / "docs" / "architecture" / "DEAP_FLIGHT_SYSTEMS_SAFETY_CONCEPT_PAPER.md").exists()
    assert (REPO_ROOT / "docs" / "architecture" / "DEAP_SYSML_V2_SAFETY_MODEL_SPECIFICATION.sysml").exists()
    assert (REPO_ROOT / "docs" / "architecture" / "DEAP_SYSML_V2_SAFETY_MODEL_SPECIFICATION.md").exists()

def test_concept_paper_contains_regulatory_frameworks():
    concept_paper = (REPO_ROOT / "docs" / "architecture" / "DEAP_FLIGHT_SYSTEMS_SAFETY_CONCEPT_PAPER.md").read_text()
...
def test_sysml_v2_model_contains_packages():
    sysml_model = (REPO_ROOT / "docs" / "architecture" / "DEAP_SYSML_V2_SAFETY_MODEL_SPECIFICATION.sysml").read_text()
